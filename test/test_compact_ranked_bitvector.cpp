...
struct sequence_initialization {
    sequence_initialization()
    {
        ...
        quasi_succinct::compact_ranked_bitvector::write(bvb,
                                                        seq.begin(),
                                                        universe, seq.size(),
                                                        params);
                        ...
                        sequence_initialization)
{
    ...
    quasi_succinct::compact_ranked_bitvector::offsets of(0,
                                                         universe, seq.size(),
                                                         params);
    uint64_t rank = 0;
                        ...
                        sequence_initialization)
{
    ...
    test_sequence(quasi_succinct::compact_ranked_bitvector(), params, 1, short_seq);
    short_seq[0] = 1;
    test_sequence(quasi_succinct::compact_ranked_bitvector(), params, 2, short_seq);
}

BOOST_FIXTURE_TEST_CASE(compact_ranked_bitvector_enumerator,
                        sequence_initialization)
{
    quasi_succinct::compact_ranked_bitvector::enumerator r(bv, 0,
                                                           universe, seq.size(),
                                                           params);
    test_sequence(r, seq);
}
